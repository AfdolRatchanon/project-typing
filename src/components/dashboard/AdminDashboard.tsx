        ...
        const unsubscribe = onSnapshot(collection(db, 'users'), async (snap) => {
            try {
                ...
                await Promise.all(snap.docs.map(async (userDoc) => {
                    ...
                    const stats: { [levelId: string]: LevelStats } = {};
                    statsSnap.forEach(d => { stats[d.id] = d.data() as LevelStats; });

                    loadedUsersData[userDoc.id] = {
                        profile: { ...profile, uid: userDoc.id },
                        stats,
                    };
                    initialImageErrors[userDoc.id] = userImageErrors[userDoc.id] || false;
                }));
