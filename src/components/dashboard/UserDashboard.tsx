...
const UserDashboard: React.FC<UserDashboardProps> = ({ user, setShowUserDashboard }) => {
    ...
    useEffect(() => {
        ...
        let statsData: { [levelId: string]: LevelStats } = {};
        let profileLoaded = false;
        let statsLoaded = false;

        const trySetData = () => {
            if (!profileLoaded || !statsLoaded) return;
            if (profileData) {
                setUserData({ profile: profileData, stats: statsData });
            } else {
                setUserData({
                    profile: { uid: user.uid, displayName: user.displayName || 'N/A', email: user.email || 'N/A', role: 'student', createdAt: Date.now() },
                    stats: {}
                });
            }
            setLoading(false);
        ...
        const unsubProfile = onSnapshot(doc(db, 'users', user.uid), (snap) => {
            profileData = snap.exists() ? { uid: user.uid, ...(snap.data() as Omit<UserProfile, 'uid'>) } : null;
            profileLoaded = true;
            trySetData();
        }, (err) => {
        ...
        const unsubStats = onSnapshot(collection(db, 'users', user.uid, 'stats'), (snap) => {
            statsData = {};
            snap.forEach(d => { statsData[d.id] = d.data() as LevelStats; });
            statsLoaded = true;
            trySetData();
        });
