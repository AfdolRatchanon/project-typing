// src/components/prepost/PrePostTestCreate.tsx

import React, { useState } from 'react';
import { X, Save, AlertTriangle } from 'lucide-react';
import type { PrePostTest, ExamSet, TestType, SetAssignmentMethod } from '../../types/types';
import ExamSetEditor from './ExamSetEditor';

interface Props {
    classroomId: string;
    teacherUid: string;
    existingTests: PrePostTest[];
    initial?: PrePostTest;
    onClose: () => void;
    onSave: (data: Omit<PrePostTest, 'testId' | 'createdAt'>) => Promise<void>;
}
...
const PrePostTestCreate: React.FC<Props> = ({
    classroomId, teacherUid, existingTests, initial, onClose, onSave,
}) => {
    const [title, setTitle] = useState(initial?.title ?? '');
    const [type, setType] = useState<TestType>(initial?.type ?? 'pre');
    const [pairId, setPairId] = useState<string>(initial?.pairId ?? '');
    ...
    const preTests = existingTests.filter(t => t.type === 'pre' && (!initial || t.testId !== initial.testId));
    const filledCount = examSets.filter(s => s.text.trim().length > 0).length;

    const handleSave = async () => {
        if (!title.trim()) { setError('กรุณาใส่ชื่อการทดสอบ'); return; }
            ...
            <div
                ...
                <div className="flex items-center justify-between p-5 pb-3" style={{ borderBottom: '1px solid var(--color-border)' }}>
                    <h3 className="text-base font-bold" style={{ color: 'var(--color-text)' }}>
                        {initial ? 'แก้ไขการทดสอบ' : 'สร้างการทดสอบใหม่'}
                    </h3>
                ...
                <div className="overflow-y-auto p-5 flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            placeholder="เช่น Pre-test ภาคเรียน 1/2568"
                        />
                    </div>
...
export default PrePostTestCreate;
